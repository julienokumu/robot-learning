...
class UR5eEnv(gym.Env):
    def __init__(self, render_mode=None):
        ...
        self.observation_sapce = gym.spaces.Box(low=-np.inf, high=np.inf, shape=(12, ), dtype=np.float32)
    ...
    def step(self, action):
        ...
        print(f"Target: {self.target}, EE pos: {self._get_end_effector_pos()}, Reward: {reward:.4f}")
        done = False
        truncated = False
...
model = PPO.load("UR5e PPO Tuned/UR5ePPO_policy_tuned.zip")
